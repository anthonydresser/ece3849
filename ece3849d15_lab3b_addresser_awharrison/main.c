...
void ADC_ISR(void) {
	...
	while((ADC0_SSFSTAT0_R & ADC_SSFSTAT0_EMPTY) == 0){
...
//			DrawString(5, 0, "7.299Hz", 15, false);
		...
		freq_whole = g_frequency/1000;
		freq_dec = g_frequency - freq_whole*1000;
		usprintf(pcStr, "f = %d.%dHz", freq_whole, freq_dec);
		DrawString(0, 86, pcStr, 15, false); // draw string to frame buffer
...
//			fft_buffer[i] = log10(out[i].r*out[i].r + out[i].i*out[i].i)*DB_SCALE + DB_OFFSET; // first fft
			fft_buffer[i] = (log10(out[i].r*out[i].r + out[i].i*out[i].i)/2)*DB_SCALE + DB_OFFSET; // better fft?
		}
		Semaphore_post(sem_display);
	}
}
